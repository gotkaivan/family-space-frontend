...
const CreateUpdateModal: FC<IProps> = ({ onCreateUpdateTask, deleteSubtask, close, data, id, statusId }) => {
	...
	const loadDefaultSubtask = () => {
		if (!data?.subtasks.length) {
			setState({ ...state, subtasks: [new SubTask()] });
		}
	};
		...
		return (
			<div className="w-full flex items-center gap-2.5">
				<Input
					...
					value={state.subtasks[0].content}
					onChange={e => changeSubtask(e, state.subtasks[0].id)}
					withError={false}
					right={
						<SquareButton
	...
	const tasks = useMemo(() => {
		if (state.subtasks.length < 2) {
			return null;
		} else {
			return state.subtasks.map((subtask, idx) => {
				if (idx > 0)
					return (
						<div
							key={subtask.id}
