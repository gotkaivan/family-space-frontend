...
const DragList: FC<IProps> = ({ data, setData, addNewCard, addNewColumn }) => {
	const formatData = useCallback((data: ITaskColumn[]) => data.reduce((acc, col) => ({ ...acc, [col.id]: col.items }), {}), []);
	...
	const removeFromList = (list: ITask[], index: number) => {
		const result = Array.from(list);
		const [removed] = result.splice(index, 1);
		return [removed, result];
	};

	const addToList = useCallback((list: ITask[], index: number, element) => {
		const result = Array.from(list);
